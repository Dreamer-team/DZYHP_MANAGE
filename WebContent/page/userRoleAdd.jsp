...
<body>
  ...
  <table width="100%" border="1" align="center" id="tb" cellpadding="0" cellspacing="0" bgcolor="#AEDEF4"
    ...
    <tr>
      ...
      <c:forEach var="role" items="${list }">
         <td style="text-align:center;"><input type="checkbox" class="choice"></td>
        <td class="val">${role.roleId }</td>
        <td>${role.roleName }</td>
        <td>${role.remark }</td>
       </c:forEach>
    </tr>
  ...
  <table width="50%" border="1" align="left" id="tb" cellpadding="0" cellspacing="0" style="border:1px solid #AEDEF4">
    <input class="btn" type="button" value="保存" onclick="" style="width:40px; height:40px">
    <input class="btn" type="button" value="返回" onclick="javascript:history.go(-1);" style="width:40px; height:40px">
  </table>
  ...
  <script>
    ...
    btn.onclick = function () {
      ...
      for (let k = 0; k < choice.length; k++) {
        if (choice[k].checked == true) {
          arr.push(val[k].innerHTML);
          value = k;
          num++;
        }
      }
      if(num == 1){
        ...
        window.location.href = "#";  //传值  val[value].innerHTML
      }else if(num>1){
        // console.log("要传多个数据");
        console.log(arr);
        window.location.href = "#"  //传值   数组arr
      }else{
        alert("请至少选择一个身份");
      }
    }
